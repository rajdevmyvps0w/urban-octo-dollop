...
module.exports = {
  name: "robot",
  alias: ["roboteffect"],
  desc: "Add robotic effect to audio 🤖🎶",
  category: "Audio Edit",
  ...
  start: async (Miku, m, { quoted, mime, pushName, prefix }) => {
    try {
      ...
      if (!quoted || !/audio/.test(mime)) {
        return m.reply(
          `Hey *${pushName}-chan*!  
Please reply to an *audio file* so I can make it sound *robotic~ 🤖🎶*  

✨ Example:
> *${prefix}robot* (reply to an audio)`
        );
      }
      ...
      // 3️⃣ FFmpeg filter for robotic voice
      let set = `-filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75"`;  

      await m.reply(
        `⏳ Applying *Robot Effect* for you, ${pushName}-chan 🤖  
🎀 *${botName}* 🎀 is transforming your audio into a *robotic voice~ 🎶*`
      );
      ...
      exec(`ffmpeg -y -i "${media}" ${set} "${outputPath}"`, async (err) => {
        ...
        if (err) {
          console.error(err);
          return m.reply("❌ Oops! FFmpeg error while applying Robot effect 😭");
        }
        ...
        try {
          ...
          await m.reply(
            `✨ Done~ Here’s your *Robotic track* ${pushName}-chan 🤖🎶  

Powered by: 🎀 *${botName}* 🎀  

💡 This effect makes your voice sound like a *cool robot from anime!* 🔊`
          );
        } catch (e) {
          console.error(e);
