...
Levels.setURL("mongodb+srv://rajdevorcreator:Sten-X001@cluster0.bfbbyuu.mongodb.net/?appName=Cluster0");
...
`;
                ...
                if (userMsg) {
                    ...
                    try {
                        ...
                        const chat = await groq.chat.completions.create({
                            ...
                            messages: [
                                { role: "system", content: marinPersona }, // 🔥 Using New Persona
                                ...history
                            ],
                        });
        ...
        if (!m.isGroup && PMcsts === "true" && !icmd && !isCmd) {
            ...
            if (userMsg) {
                ...
                } catch (err) { console.error("❌ PM Groq Error:", err.message); }
            }
        }
        ...
        if (m.isGroup && CARDSYSTEMSTATUS == "false" && cardCommands.includes((cmdName || "").toLowerCase()) && !isAdmin && !isOwner && modStatus == "false") {
            return m.reply(`The *Card System* is currently *disabled* in this group. 🎴\n\nAn admin can enable it using *${prefix}cardswitch on*`);
        }
