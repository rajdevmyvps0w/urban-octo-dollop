...
Complete guide for using API utilities, helpers, and external integrations in Runa (ルナ) – 月の光.
...
Runa (ルナ) – 月の光 uses a robust fallback system that tries multiple API endpoints until one succeeds.
    ...
    for (const url of endpoints) {
        try {
            const response = await fetch(url, {
                headers: {
                    "User-Agent": "Runa (ルナ) – 月の光-Bot/1.0",
                },
            });
