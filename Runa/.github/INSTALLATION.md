# 📦 Runa (ルナ) – 月の光 Installation Guide

Complete installation guide for setting up Runa (ルナ) – 月の光 WhatsApp bot on various platforms.
...
```bash
# Using HTTPS
git clone https://github.com/naruyaizumi/Runa (ルナ) – 月の光.git

# Or using SSH
git clone git@github.com:naruyaizumi/Runa (ルナ) – 月の光.git
...
```bash
# Create service file
sudo nano /etc/systemd/system/Runa (ルナ) – 月の光.service
```
...
Type=simple
User=youruser
WorkingDirectory=/path/to/Runa (ルナ) – 月の光
...
# View logs
sudo journalctl -u Runa (ルナ) – 月の光 -f
```
...
services:
    Runa (ルナ) – 月の光:
        build: .
        container_name: Runa (ルナ) – 月の光-bot
...
# View logs
docker-compose logs -f Runa (ルナ) – 月の光
...
2. Upload Runa (ルナ) – 月の光 files via SFTP
...
# If using systemd
sudo chown -R $USER:$USER /path/to/Runa (ルナ) – 月の光
```
...
# View real-time logs
tail -f logs/Runa (ルナ) – 月の光.log  # If using file logging

# PM2 logs
pm2 logs Runa (ルナ) – 月の光 --lines 100

# Systemd logs
sudo journalctl -u Runa (ルナ) – 月の光 -f --since "10 minutes ago"
```
...
- **GitHub Issues**: [Report Bug](https://github.com/naruyaizumi/Runa (ルナ) – 月の光/issues)
